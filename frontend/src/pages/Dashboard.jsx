...
export default function Dashboard() {
  ...
  return <main className="dashboard"><header className="topbar"><div className="brand-mark">A<span>/</span>O</div><button className="text-button" onClick={logout}>Sign out <span aria-hidden="true">↗</span></button></header><section className="dashboard-content"><p className="eyebrow">Your private space</p><section className="welcome-section" aria-labelledby="welcome-heading"><h1 id="welcome-heading">Welcome back, <em>{user.name}</em>!</h1></section><div className="account-detail"><span className="detail-label">Account details</span><div><strong>{user.email}</strong><span>Member since {new Date(user.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</span></div></div><div className="account-detail"><span className="detail-label">Deployment status</span><div><strong>{cicdStatus}</strong>{cicdMessage && <span>{cicdMessage}</span>}</div></div></section></main>
}